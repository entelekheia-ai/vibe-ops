...
the plugin — ten policy references, thirty-two scaffold templates, three shell scripts and one pinned
agent live only in the plugin tree, reachable only through `${CLAUDE_PLUGIN_ROOT}`. This RFC moves them:
three governance types are added (`style`, swappable like an output style; `instructions`, the
instruction surface; `knowledge`, absorbing `log`), the remaining references join the packages whose
policy they are, the scaffold becomes a composition of what each activated governance ships, and the
three scripts become `vibe-ops hook` surfaces. What stays in the plugin is what needs judgement or is
...
- **Thirty-two scaffold templates** (`plugin/skills/setup/templates/`, 124K) are copied by a pinned
  ...
  legacy file.
- **Three shell scripts** (`plan-progress-nudge.sh`, `session-state-cleanup.sh`,
  `session-touched-repos.sh`, 389 lines) are the last hooks that do not call `vibe-ops hook <surface>`;
  seven of nine registrations already do.
- **One pinned agent**, `scaffolder`, exists only because the copy-and-substitute step had no
  deterministic surface (ADR-0013 records the pin and its verifier).
...
| `instructions` | the instruction surface — `AGENTS.md`, its `CLAUDE.md` import, the `.agents/` ↔ `.claude/` bridge, `repo-guardrails.md`; its policy (`instruction-surfaces@1`), its scaffold files (`root/CLAUDE.md`, `agents/rules/repo-guardrails.md`, `agents/skills/.gitkeep`), and its sensor (the `agents-md` ops composes this package's gates) | `plugin/references/instruction-surfaces.md`, four scaffold files |
| `knowledge` | what a piece of work taught: `project/log/` (a trap, addressed by path) **and** `project/learnings/` (a fact that holds beyond one repository); the promotion test (`knowledge-lifecycle@1`); the `new-log` verb and the log index; the `log` type's template, migrations and fragment | `governance-log` (absorbed), `plugin/references/knowledge-lifecycle.md` |
...
Each keeps its `vibe-ops-reference: <name>@N` stamp and its version history. `records norm` gains a
`policy` facet; a skill reads `vibe-ops records norm --type base --facet policy/convergence --print` where
it read a plugin path.
...
from the activated types' lifecycles, not copied; `GOVERNANCE.md` likewise (open question). The
placeholder table is data in each package; the substitution is one function. The `scaffolder` agent and
the pin ADR-0013 records for it retire: there is nothing left for a model to copy.
...
`plan-progress-nudge.sh` becomes `vibe-ops hook plan-progress`, owned by `governance-plan` (it is about
plans); `session-state-cleanup.sh` and `session-touched-repos.sh` become surfaces of the CLI's `hook`
namespace. `hooks.json` then names `vibe-ops` on nine of nine registrations, and the plugin ships no
script. The behaviour test that exists for the nudge keeps running against the CLI surface.
...
- A repository keeps working with no change: the default bindings gain `style`, `instructions` and
  `knowledge` beside the five record types; `log` resolves to `knowledge`'s log facet.
...
- The `dogfooding-drift` check's template pairs empty; the check stays for the pairs that remain.
...
2. `governance-knowledge`: package, `type.json`, the log facets moved from `governance-log`, the
   learnings facet, `knowledge-lifecycle` as policy; `governance-log` retired with a supersession pointer.
3. `governance-style`: the type, the default package, `types.style` in the default bindings; the four
   authoring skills read it through the CLI.
4. `governance-instructions`: the type, its scaffold files, `instruction-surfaces` as policy; the
   `agents-md` ops composes its gates.
5. `setup scaffold` and `harness install`; the `scaffolder` agent and its ADR-0013 pin retired;
   `35-dogfooding-drift` pairs emptied.
6. The three scripts as `hook` surfaces; `hooks.json` names `vibe-ops` nine times.
...
- Whether `ops-agents-md` becomes part of `governance-instructions` or stays a separate ops that composes
  that package's gates (the ops/governance split RFC-0001 draws says: stays).
