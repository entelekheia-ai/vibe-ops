...
The decision comes in two steps, and they are different questions.

**First: should this be an `AGENTS.md` at all?** A **path-scoped rule** (`paths: ["that-dir/**"]`) under
`.agents/rules/` is usually the better answer for a guardrail — it loads exactly when work touches those
files and costs nothing otherwise. Reach for a nested `AGENTS.md` when the content is a *map* of a
subtree that someone working inside it needs whole, not a rule that fires on a path.

**Then, having decided to write one: every `AGENTS.md` MUST have a sibling `CLAUDE.md` containing
`@AGENTS.md`.** This holds at a repository root and at every nesting level, and it is not a style
preference — it is the difference between a file that loads and one that does not. Measured across ten
repositories on 2026-08-10: eleven `AGENTS.md` files had no sibling and no `@`-import reaching them, and
two of those were **repository roots**, where the map nothing loads is the first file anyone assumes was
read.

An `@`-import from an ancestor that does load is the documented alternative, for a layout where the
sibling file would be noise. It carries a cost the sibling does not: the import is in a different file
from the one it loads, so moving or renaming the target breaks it silently.
