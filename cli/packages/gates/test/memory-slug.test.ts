...
test("exactly one hit — the real slug — and neither decoy counts", async () => {
  ...
  const outcome = await memorySlug.run({ repoRoot, pluginDir: repoRoot, files: ["decoys.md"], options: {} });
...
test("a clean file passes and is counted as examined", async () => {
  ...
  const outcome = await memorySlug.run({
    repoRoot,
    pluginDir: repoRoot,
    files: ["AGENTS.md"],
    options: {},
...
test("a shipped template is not examined — it is written to resolve in the target repo", async () => {
  ...
  const outcome = await memorySlug.run({
    repoRoot,
    pluginDir: repoRoot,
    files: ["skills/demo/templates/demo.md"],
    options: {},
  });
  assert.deepEqual(outcome.findings, []);
