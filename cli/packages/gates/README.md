...
| Gate | Ported from | What it catches |
|---|---|---|
| `budget` | `module-check/sh/checks/10-budget.sh` | `AGENTS.md` over its line budget |
| `pairing` | new | an `AGENTS.md` with no sibling `CLAUDE.md` containing `@AGENTS.md` |
| `bridge` | `module-check/sh/checks/30-bridge.sh` | a `.claude/` entry that is not a resolving relative symlink |
| `check-frontmatter` | `40-frontmatter.sh` + `45-skill-frontmatter.sh` | a missing frontmatter block, description, or (schema `skill`) an unquoted `": "` that silently drops all fields |
| `memory-slug` | `60-memory-slugs.sh` | a `[[…]]` personal-memory link, fences and inline code spans excluded |
