...
function parseWithGrammar(text: string, language: Parser.Language): Parser.Tree {
  ...
  return {
    get(file: string): Document {
      ...
      try {
        ...
        const document: Document = { file, text: "", uncovered: `cannot read file: ${String(cause)}` };
      ...
      const document: Document = {
        ...
        tree: parseWithGrammar(text, grammar.language),
      };
      cache.set(file, document);
      return document;
    },
