...
| [`agents/<name>.md`](agents/) | Subagents the skills dispatch to. **Auto-discovered — no manifest entry**, and that is the tested shape: with no `agents` key in `plugin.json`, `--plugin-dir` lists the file as `vibe-ops:governance-auditor` (measured 2026-08-13); the manifest's `agents` field takes an *array of file paths*, not a directory, so declaring it would trade auto-discovery for one entry per agent. Three today. Two are read-only for the same reason — [`governance-auditor`](agents/governance-auditor.md), which every target-state skill delegates its survey to ([`convergence-policy.md`](references/convergence-policy.md)), and [`migration-rehearser`](agents/migration-rehearser.md), which walks a migration note against the artifact it handles worst: neither holds a writing tool, so "writes nothing" is enforced rather than promised. [`scaffolder`](agents/scaffolder.md) is the exception and the only pinned surface — it writes, decides nothing, and is `haiku` because `setup`'s own `grep '{{'` and Step 7 checklist go red within the minute ([ADR-0013](../project/adr/0013-the-model-a-shipped-plugin-may-pin.md)). Which fields are set and which are deliberately refused — `skills` preloads rather than restricts, `maxTurns` on a surveyor truncates into a plausible answer, `isolation: worktree` would survey the committed tree instead of yours — is [Plan-024](../project/plans/shipped/024-the-work-a-skill-declares-and-the-caller-cannot-afford.md)'s Decision Log. |
...
| [`types/`](types/) | One folder per record type, each holding a `type.json` that DECLARES where that type's facets are — its template, its authoring rules, its migration notes — plus its header schema, numbering and candidate directories. A manifest, not a folder the facets moved into: a path here is load-bearing (the authoring rules declare their version *as* their path), so the units point at what already exists. `index.json` beside them is **generated** from the manifests and verified by the `type-index-drift` gate, which can regenerate it with `--fix`; never edit it by hand. [Plan-030](../project/plans/030-the-type-unit-and-the-compositions-derived-from-it.md) Track 3 moves this whole directory into an npm package — the norm is meant to travel through npm, not through the plugin ([RFC-0003](../project/rfc/0003-a-governance-type-as-a-pluggable-unit.md)). |
| [`hooks/`](hooks/) | The only always-on surface — auto-discovered from `hooks.json`, no manifest entry. A hook fires in **every** repository where the plugin is installed, so each carries its own cheap exit as its first act. **Seven of the nine registrations name `vibe-ops` directly and ship no script** ([`docs/reference/hook-surfaces.md`](../docs/reference/hook-surfaces.md) is the catalogue; [`docs/how-to/write-a-hook.md`](../docs/how-to/write-a-hook.md) is the recipe). Why this surface exists: [`positioned-context-and-hooks.md`](../project/research/positioned-context-and-hooks.md). |
...
| [`scripts/`](scripts/) | **Runtime**, shipped and run inside a target repo — down to one. `session-touched-repos.sh` is what the `Stop` hook calls to attribute a turn's writes to a repository from the session transcript, never from `cwd`; it is session bookkeeping, which is why Plan-011 left it here while every governance script became a CLI verb. |
