...
| [`references/`](references/) | **Emptying.** Shared policy the skills point at instead of restating, each declaring `vibe-ops-reference: <name>@<integer>` in its frontmatter — so which closures ran under which rule is a query rather than an archaeology. Since Plan-040 a policy belongs to the package whose policy it is and is read through the CLI (`vibe-ops records norm --facet policy --name <n>`, `vibe-ops records norm --type style --facet policy --for <target>`, `vibe-ops harness policy --name <n>`). **Nine of the ten are gone and only this directory's own README remains** — Track 6 removes it with the directory. A file added here now is a file with no owner. |
