...
| [`references/`](references/) | **Emptying.** Shared policy the skills point at instead of restating, each declaring `vibe-ops-reference: <name>@<integer>` in its frontmatter — so which closures ran under which rule is a query rather than an archaeology. Since Plan-040 Track 1 a policy belongs to the package whose policy it is and is read through the CLI (`vibe-ops records norm --facet policy --name <n>`, `vibe-ops harness policy --name <n>`); four files wait here for the types that will own them — `authoring-style` for `style`, `instruction-surfaces` for `instructions`, `knowledge-lifecycle` for `knowledge`. A file added here now is a file with no owner. |
