...
description: Write or clean up a README — repo or package — as pure presentation and usage, not process history. Use when scaffolding a new repo/package, when editing or adding a section to a README that already exists, when one has drifted into a decision log or status narrative, or when the user asks to write/fix/review a README.
...
**Scale the pass to the edit.** This skill loads whenever a `README.md` is opened, so most of the time it
arrives for one added paragraph, not a rewrite. Then only Step 3 applies — the addition is presentation and
usage, or it belongs in an ADR, `project/log/` or the changelog and does not go here — plus the rule that
it joins the section that already owns its subject. Steps 0 through 2 are for an explicit invocation, an
absent README, or one whose opening you have just found to be untrue. **Never open Step 0's three questions
for a one-paragraph edit.**
