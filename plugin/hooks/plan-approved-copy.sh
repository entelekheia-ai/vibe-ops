...
# Status row is what `vibe-ops hook plan-context` asks for only when the turn intends a
...
# when `vibe-ops hook plan-context` detected the session's project root was not the
