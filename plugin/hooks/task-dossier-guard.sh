...
# and everything it taught is promoted during /vibe-ops:close's task branch.
# Deleting it by hand loses that silently, which is the failure close exists to
# prevent and could not prevent, because prose cannot fire at the moment of the act.
#
...
#    done."
# This hook is a reader for that box. Ticking it is close's Step 6, so the
