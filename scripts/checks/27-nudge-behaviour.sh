...
check_nudge_behaviour() {
  head_
  local id="nudge-behaviour" hook="$ROOT/hooks/plan-progress-nudge.sh"
  ...
  if [ ! -f "$ROOT/scripts/resolve-governance.sh" ] || [ ! -f "$ROOT/scripts/session-touched-repos.sh" ]; then
